...
/**
 * Magecom_Sampleorder_IndexController class
 *
 * @category Magecom
 * @package Magecom_Sampleorder
 * @author  Magecom
...
class Magecom_Sampleorder_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    private function _createSampleProduct($product)
    {
        ...
        if ($sampleProduct->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_SIMPLE && $sampleProduct->getHasOptions() == 1) {
            $this->unsetCustomOptionsPrice($sampleProduct);
        }

        $previosStore = Mage::app()->getStore()->getStoreId();
        ...
        Mage::app()->setCurrentStore($previosStore);
     ...
     * @param  array $param
     *
     * @return array $newOptionArray option for new sample product,
     * created on the basis of custom options of parent product
     */
